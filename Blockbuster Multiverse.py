        ...
        if(item["name"] == item_name):
            ...
            if(item["type"] == "door"):
                ...
                if(have_key == True and weapon == True):
                    print("You manage to unlock it, you can feel the fire roaring within the Volcano.\n")
                    input("Press Enter to continue...")    
                    next_room = get_next_room_of_door(item, current_room)                    
                if(have_key == True and weapon == False):
                    print("You manage to unlock it, you can see the next universe through the portal.\n")                    
                    next_room = get_next_room_of_door(item, current_room)
...
# Amelies groot and key challenge code
                    ...
                    if result_game_root_key==1:
                        item_found = object_relations[item["name"]].pop()
                        game_state["keys_collected"].append(item_found)
                        output += "Congratulations! You win! You got the " + item_found["name"] + "."
...
#Sword Riddle
                    ...
                    if result_game_sword==1:                           
                        ...
                        os.system('cls') 
                        print_sword()
                        output += "Got it, you now yield " + item_found["name"] + ", the King's Sword."                        
...
def game_ring():    
    ...
    os.system('cls')
    print_ending()
    answer=input('''The One ring, and the destiny of Middle Earth lies in your hands!\nWill you destroy the ring in the volcano? (yes/no): \n''')
    if answer.lower()=='yes':
        os.system('cls')
        print_gandalf()
...
def game_key_b(): #Here is the challege of Alicia 
    ...
    #mixer.music.play()
    print_mad_hatter() 
    answer=input('No room in this party! It’s very rude to sit down without being invited.\nDo you want to ask me something? ? (yes/no): \n')
    if answer.lower()=='yes':
...
def game_key_c(): #Here is the challege of Vanessa
    ...
    answer = input('Are you sure you want to fight against me muggle? (yes/no):\n')
    if answer.lower()=='yes':
        os.system('cls') 
        print_voldemort()
...
def game_root_key(): #Here is the challege of Amelie  
    ...
    answer=input('I can give it to you if you play a game with me. Are you ready? (yes/no): \n')
    if answer.lower()=='yes':
        os.system('cls') 
        print_groot()
        answer=input('With whom in the room have I been a bounty hunter before we met starlord?\n')
